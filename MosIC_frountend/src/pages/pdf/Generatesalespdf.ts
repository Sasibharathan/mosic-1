...
const cur = (v: unknown) => num(v).toLocaleString("en-IN", { minimumFractionDigits: 2 });
...
export async function generateSalesPDF(
  ...
  if (prefetchedRow && prefetchedItems) {
    ...
    try {
      ...
      if (status === 403) throw new Error(`Access denied fetching Sales record #${docId}.`);
      if (status === 401) throw new Error("Session expired. Please log in again.");
      if (status === 404) throw new Error(`Sales record #${docId} was not found.`);
      throw err;
    }
  ...
  d.setFontSize(11);
  d.setTextColor(255, 255, 255);
  d.text(docTypeLabel(row.salesDoctype), pw / 2, ctx.y + 6.2, { align: "center" });
  ctx.y += 13;
  ...
  if (row.salesAddressedTo) {
    ...
    d.setFont("helvetica", "normal");
    d.setTextColor(20, 20, 20);
    d.text(fmt(row.salesAddressedTo), ml + 36, ctx.y + 5.5);
    ctx.y += 8;
  }
  ctx.y += 4;
  ...
  d.setFontSize(7.5);
  d.setTextColor(60, 60, 60);
  if (row.salesPaymentTerms)  d.text(`Payment Terms  : ${row.salesPaymentTerms}`,  ml + 2, ctx.y + 11);
  if (row.salesDeliveryTerms) d.text(`Delivery Terms : ${row.salesDeliveryTerms}`, ml + 2, ctx.y + 16);
  if (row.salesDescription)   d.text(`Description    : ${row.salesDescription}`,   ml + 2, ctx.y + 21);
  ctx.y += termsH + 4;
  ...
  doc.save(`${typeLabel}_${row.id}_${(row.salesToParty || "party").replace(/[^a-zA-Z0-9]/g, "_").slice(0, 20)}.pdf`);
}