...
const ITEMS_URL = "/api/purchase-items";
...
const createEmptyItem = (refFileNo: number): PurchaseItemDTO => ({
  id: null,
...
// ─── PDF Generator ────────────────────────────────────────────────────────────
// Mirrors printSalesPDF — builds a styled HTML doc in a hidden iframe and prints.

function printPurchasePDF(
  purchaseRecord: PurchaseRecord,
  items: PurchaseItemDTO[],
  grandTotals: { taxable: number; cgst: number; sgst: number; igst: number; total: number }
) {
  const docLabel = DOCTYPE_LABELS[purchaseRecord.purchaseDoctype] ?? purchaseRecord.purchaseDoctype;
  const currency = purchaseRecord.purchaseCurrency || "";

  // Strip "id-label" stored format → plain label
  const getFileRefString = (): string => {
    const raw = purchaseRecord.purchaseFileRef ?? "";
    if (!raw) return "";
    const dashIdx = raw.indexOf("-");
    return dashIdx !== -1 ? raw.slice(dashIdx + 1) : raw;
  };
  const fileRefDisplay = getFileRefString();
...
<head>
  <meta charset="UTF-8"/>
  <title>${docLabel} — Purchase #${purchaseRecord.id}</title>
  <style>
...
<body>
  <div class="header">
    ...
    <div>
      ...
      <div class="doc-meta">Purchase Record #${purchaseRecord.id}</div>
      <div class="doc-meta">Date: ${purchaseRecord.purchaseDate || "—"}</div>
  ...
  <div class="parties">
    <div class="party-block">
      <h4>From</h4>
      <p>${purchaseRecord.purchaseFromParty || "—"}</p>
    </div>
    <div class="party-block">
      <h4>To</h4>
      <p>${purchaseRecord.purchaseToParty || "—"}</p>
    </div>
  </div>

  ${purchaseRecord.purchaseDescription ? `<div style="margin-bottom:20px;font-size:12px;color:#444;background:#fffbe6;border:1px solid #ffe58f;border-radius:6px;padding:12px;"><strong>Description:</strong> ${purchaseRecord.purchaseDescription}</div>` : ""}
...
export default function PurchaseDetailsItems({ purchaseRecord, onClose }: Props) {
  const [items, setItems]                   = useState<PurchaseItemDTO[]>([]);
  const [formState, setFormState]           = useState<PurchaseItemDTO>(createEmptyItem(purchaseRecord.id));
  ...
  const setField = <K extends keyof PurchaseItemDTO>(field: K, value: PurchaseItemDTO[K]) =>
  ...
  const fetchItems = useCallback(async () => {
    ...
    clearMsgs();
    try {
      const res = await axiosInstance.get<PurchaseItemDTO[]>(
        `${ITEMS_URL}/by-ref/${purchaseRecord.id}`
      );
      setItems(res.data);
    } catch (err) {
  ...
  const openCreateForm = () => {
    setEditingId(null);
    setFormState(createEmptyItem(purchaseRecord.id));
    clearMsgs();
    setShowForm(true);
  };

  const openEditForm = (item: PurchaseItemDTO) => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const payload: Partial<PurchaseItemDTO> = { ...formState };
  ...
  const handleDownloadPDF = () => {
    ...
    printPurchasePDF(purchaseRecord, items, grandTotals);
  };
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4 border-b border-gray-100 pb-4 dark:border-gray-800">
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2">
            ...
            <DoctypeBadge doctype={purchaseRecord.purchaseDoctype} />
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            <span className="font-medium text-gray-700 dark:text-gray-300">
              {purchaseRecord.purchaseFromParty}
            </span>
            {" → "}
            <span className="font-medium text-gray-700 dark:text-gray-300">
              {purchaseRecord.purchaseToParty}
            </span>
            {purchaseRecord.purchaseDate && (
              <span className="ml-2 text-xs text-gray-400">({purchaseRecord.purchaseDate})</span>
            )}
            {purchaseRecord.purchaseFileRef && (
      ...
      >
        ...
        <form onSubmit={(e) => void handleSubmit(e)} className="mt-6 flex flex-col gap-5">
          ...
          {/* Row 2 — Qty, Unit, Rate, Taxable Value */}
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <div>
              <Label htmlFor="itemTotal">
                Total <span className="text-xs text-gray-400">(auto)</span>
              </Label>
      ...
      <div className="overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-800">
        <Table>
          <TableHeader className="bg-gray-50 dark:bg-white/[0.03]">
            <TableRow>
              {["#", "Product / Service", "HSN", "Qty", "Unit", "Rate", "Taxable Value",
                "CGST %", "CGST Amt", "SGST %", "SGST Amt", "IGST %", "IGST Amt", "Total", "Actions"
              ].map((h) => (
                <TableCell
                  key={h}
                  isHeader
          ...
          <TableBody className="divide-y divide-gray-100 dark:divide-gray-800">
            ...
            {!isLoading && items.map((item, idx) => (
              <TableRow key={item.id} className="transition-colors hover:bg-brand-50/60 dark:hover:bg-brand-500/5">
                <TableCell className="px-4 py-3 text-sm text-gray-400">{idx + 1}</TableCell>
                <TableCell className="max-w-[180px] px-4 py-3 text-sm font-medium text-gray-800 dark:text-white/90">
                  <span className="block truncate">{item.nameOfProductService || "—"}</span>
                </TableCell>
          ...
          {[
            ...
            <div key={card.label} className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
              ...
              <p className={`mt-1 text-lg font-bold ${card.color}`}>
                {purchaseRecord.purchaseCurrency && (
                  <span className="mr-1 text-sm font-normal opacity-70">{purchaseRecord.purchaseCurrency}</span>
                )}
                {fmt(card.value)}
              </p>
            </div>
