...
const getErrorMessage = (error: unknown): string => {
  if (axios.isAxiosError(error)) {
    ...
    if (error.response?.status === 409) {
      return "Gmail or username already registered.";
    }
    if (error.response?.status === 401 || error.response?.status === 403) {
      return "You must be logged in to register new users.";
    }
...
export default function SignUpForm() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setTimeout(() => void navigate("/signin"), 1500);
    } catch (err) {
        ...
        <div>
          <div className="mb-5 sm:mb-8">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Create a new system user. Requires an active login session.
            </p>
          </div>

          {/* Global error / success banners */}
          {errorMessage && (
            <div className="mb-5 rounded-lg border border-error-500/40 bg-error-50 px-4 py-3 text-sm text-error-700 dark:bg-error-500/10 dark:text-error-400">
              {errorMessage}
            </div>
          )}
          {successMessage && (
            <div className="mb-5 rounded-lg border border-success-500/40 bg-success-50 px-4 py-3 text-sm text-success-700 dark:bg-success-500/10 dark:text-success-400">
              ...
              <div className="space-y-5">
                <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
                  ...
                  <div className="sm:col-span-1">
                    ...
                    />
                    {usernameError && (
                      <p className="mt-1 text-xs text-error-500">{usernameError}</p>
                    )}
                  </div>

                  {/* Contact */}
                  <div className="sm:col-span-1">
                    <Label htmlFor="reg_contact">Contact</Label>
                    <Input
                      id="reg_contact"
                      type="text"
                ...
                <div>
                  ...
                  />
                  {gmailError && (
                    <p className="mt-1 text-xs text-error-500">{gmailError}</p>
                  )}
                </div>
